...
import { createQueryStringDriver } from '../src/index'
...
describe('Query String Driver', () => {
  ...
  it('should update history when enabled', async () => {
    mockLocation.href = 'https://example.com/'
    const storage = createStorage({
      driver: createQueryStringDriver({ updateHistory: true, historyMethod: 'pushState' })
    })
  ...
  it('should use replaceState when configured', async () => {
    mockLocation.href = 'https://example.com/'
    const storage = createStorage({
      driver: createQueryStringDriver({ updateHistory: true, historyMethod: 'replaceState' })
    })
  ...
  it('should respect maxUrlLength', async () => {
    ...
    const storage = createStorage({
      driver: createQueryStringDriver({ updateHistory: true, maxUrlLength: 50 })
    })
